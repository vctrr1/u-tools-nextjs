...
function TemperatureConversionComp() {
    ...
    const [conversionResults, setConversionResults] = useState<ConversionResultTemp[]>([]);
    ...
    const convertUnits = (amount: number, fromUnit: TemperatureType) => {
        ...
        if (fromUnit === "Celsius") {
            ...
            baseAmountInCelsius = (amount - 32) / conversionRatesTemperature.Fahrenheit;
        ...
        for (const [key, rate] of Object.entries(conversionRatesTemperature)) {
            if (key === "Fahrenheit") {
                results.push({ unit: key as TemperatureType, value: (baseAmountInCelsius * rate) + 32 });
            } else if (key === "Kelvin") {
                results.push({ unit: key as TemperatureType, value: baseAmountInCelsius + 273.15 });
            } else {
                results.push({ unit: key as TemperatureType, value: baseAmountInCelsius });
            }
        }
    ...
    return (
        ...
        <div className="flex gap-2 mt-5 mb-4 w-full">
            <Input placeholder="Valor" className="w-[40%]" onChange={(e) => {
